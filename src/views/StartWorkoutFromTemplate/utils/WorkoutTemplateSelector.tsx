...
export const WorkoutTemplateSelector = ({setPreviewedTemplateId, disabled}: {setPreviewedTemplateId: (id: string) => void, disabled: boolean}) => {
    ...
    return(
        ...
        <ul>
            {templates.map(template => (
                <li key={template.id}>
                    <Button disabled={disabled} onClick={() => setPreviewedTemplateId(template.id)}>
                        {template.name}
                    </Button>
                </li>
            ))}
